...
int main()
{
  PointerType *ds=NULL,*pv=NULL,*tmp=NULL;
    ...
    if(TimViTriTruoc(ds,them.grade)==NULL)
      ds=InsertToHead(ds,them);
    else
      if(TimViTriTruoc(ds,them.grade)->Next==NULL)
	InsertToLast(ds,them);
      else
	InsertMiddle(TimViTriTruoc(ds,them.grade),them);
    Print(ds);
    ...
    gets(str);
    tmp=TimViTri(ds,str);
    if(tmp==NULL) printf("Khong co ID\n\n");
    else
