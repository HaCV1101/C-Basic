...
void EnQueueUuTien(ElementType X, Queue *Q)
{
  Node* ViTri;
  ViTri=(Node*)malloc(sizeof(Node));
  ViTri->Element=X;
  ViTri->Next=Q->Front;
  Q->Front=ViTri;
}
...
int main()
{
  ...
  do
	...
	case 6:
	  ...
	  while(1)
	    {
	      printf("Nhap ten khach hang them uu tien KHONG CO DAU CACH:");
	      gets(Khach.ten);
	      if(kiemTraTen(Khach.ten,strlen(Khach.ten))==1)
		{
		  break;
	  ...
	  while(1)
	    {
	      printf("Nhap sdt khach hang them uu tien KHONG CO DAU CACH:");
	      gets(Khach.sdt);
	      if(kiemTraTen(Khach.sdt,strlen(Khach.sdt))==1)
		{
		  break;
